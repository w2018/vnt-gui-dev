...
[vnt-dev/vnt](https://github.com/vnt-dev/vnt) 的 Windows 图形化管理工具（Rust + Tauri 2 + React）。当前版本 **v2.3.0**。
...
- **桌面共享（v2.3.0 重构）**：控制端/被控端双模、标签页收纳式布局（画面最大化）——MF H.264 低延迟编码（GOP 1s / CBR / 无 B 帧）、高分屏分辨率缩放、fps 节流、WebCodecs 硬解 + 软解自动回退（虚拟机可用）、彩色光标、点击画面聚焦后拦截键盘、连接请求全局授权弹窗（前后台/任意页面）、断开重连无黑屏
