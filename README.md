...
将官方 `vnt-cli.exe` 作为 Sidecar 嵌入，GUI 负责进程管理、配置、状态展示与流量统计。
...
- **FTP 服务**：内嵌 FTP 服务器（libunftp）、用户权限管理（上传/下载/删除/只读）、ROOT 目录选择、端口与 PASV 配置、监听地址展示、连接日志、随应用/系统自启
- **软件更新**：vnt-cli 与 GUI 双通道版本检测 + 一键更新
- **体验**：首次启动向导、深浅主题、全局快捷键
